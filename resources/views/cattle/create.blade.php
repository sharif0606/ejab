										...
										<select class="width-100" name="division_id" onchange="$('.pdist').hide();$('.pdist'+this.value).show()">
										    ...
										    @if($division)
										        @foreach($division as $divi)
										            <option value="{{$divi->id}}" @if($divi->id==old('division_id')) selected @endif class="pdiviop pdiviop{{$divi->country_id}}">{{$divi->division}}</option>
										        @endforeach
										    @endif
										</select>
									</span>
										...
										<select class="width-100" name="district_id">
										    ...
										    @if($district)
										        @foreach($district as $dist)
										            <option value="{{$dist->id}}"  @if($dist->id==old('par_district')) selected @endif class="pdist pdist{{$dist->division_id}}">{{$dist->district}}</option>
										        @endforeach
										    @endif
										</select>
									</span>
