    ...
    @endif
        ...
        <div class="widget-box">
			<div class="widget-header widget-header-small">
				<h5 class="widget-title lighter">User Add Form</h5>
			</div>
			<div class="widget-body">
				<div class="widget-main">
					<form action="{{route('users.store')}}" method="post" class="form-search">
					@csrf
						<div class="row">
								...
								<div class="form-group @if($errors->has('role_id')) has-error @endif">
									...
									<span class="block input-icon input-icon-right">
										<select class="width-100" name="role_id">
											...
											@if($role)
												@foreach($role as $r)
											<option value="{{$r->id}}" @if($r->id==old('role_id')) selected @endif>{{$r->type}}</option>
												@endforeach
											@endif
										</select>
